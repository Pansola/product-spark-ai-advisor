...
const DemandTrendChart: React.FC<DemandTrendChartProps> = ({
  ...
  if (isLoading) {
    return <div className="mt-8 h-64 sm:h-80 w-full">
        <Skeleton className="h-full w-full" />
      </div>;
  }
  return <div className="mt-8 h-64 sm:h-80 w-full my-0">
      {error && <div className="text-amber-600 text-sm mb-2 p-2 bg-amber-50 rounded-md border border-amber-100">
          {error}
        </div>}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-2 gap-2">
        ...
        {realTimeData && <div className="text-xs text-gray-500">
